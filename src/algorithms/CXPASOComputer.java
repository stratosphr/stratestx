...
public final class CXPASOComputer extends AComputer<ATS> {
    ...
    private final ArrayList<AbstractState> A;
    ...
    public CXPASOComputer(Machine machine, ArrayList<AbstractState> A) {
            ...
            for (Event e : machine.getEvents().values()) {
                ...
                do {
                    ...
                    if (result.isSAT()) {
                        ...
                        ConcreteState c = C.stream().filter(concreteState -> concreteState.getMapping().equals(cModel)).findFirst().get();
                        ConcreteState c_ = C.stream().filter(concreteState -> concreteState.getMapping().equals(c_Model)).findFirst().orElse(new ConcreteState("c" + C.size() + q_.getName(), c_Model));
                        Q.add(q_);
            ...
            for (Event e : machine.getEvents().values()) {
                for (AbstractState q_ : A.stream().filter(q_ -> !Delta.contains(new AbstractTransition(q, e, q_))).collect(Collectors.toList())) {
                    ...
                    if (result.isSAT()) {
                        ...
                        ConcreteState c = C.stream().filter(concreteState -> concreteState.getMapping().equals(cModel)).findFirst().orElse(new ConcreteState("c" + C.size() + q_.getName(), c_Model));
                        ConcreteState c_ = C.stream().filter(concreteState -> concreteState.getMapping().equals(c_Model)).findFirst().orElse(new ConcreteState("c" + C.size() + q_.getName(), c_Model));
                        Delta.add(new AbstractTransition(q, e, q_));
                        C.add(c);
                        C.add(c_);
